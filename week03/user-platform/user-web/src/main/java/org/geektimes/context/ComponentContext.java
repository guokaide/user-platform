...
public class ComponentContext {
    ...
    // JNDI namespace - java:comp/env portion
...
//    }
    ...
    public void init(ServletContext servletContext) throws RuntimeException {
        ...
        // 1. ComponentContext 注册到 ServletContext 中
        servletContext.setAttribute(CONTEXT_NAME, this);

        // 获取当前 ServletContext（WebApp）ClassLoader
        this.classLoader = servletContext.getClassLoader();

        // 2. JNDI: 获取 ENV Context
        initEnvContext();

        // 3. 通过依赖查找 JNDI Naming Context，实例化依赖查找到的对象(setter 方法的执行 by Tomcat BeanFactory)
        instantiateComponents();

        // 4. 初始化组件及组件的依赖注入等生命周期实现
    ...
    protected void instantiateComponents() {
        // 遍历获取所有的组件名称 JNDI Operations
        List<String> componentNames = listAllComponentNames();
        // 通过依赖查找，实例化对象（ Tomcat BeanFactory setter 方法的执行，仅支持简单类型）
        // JNDI 定义好之后，并不是马上初始化，而是通过 lookup 延迟初始化
    ...
    /**
     * 注解注入参考：Spring-context-support AbstractAnnotationBeanPostProcessor
     *
     * @param component
    ...
    private List<String> listAllComponentNames() {
        return listComponentNames("/"); // java:comp/env/
    }
    ...
    protected List<String> listComponentNames(String name) {
        return executeInContext(context -> {
            ...
            // 目录 - NamingContext: JNDI Naming Context
            // 节点 - NamingEntry
            ...
            while (e.hasMoreElements()) {
                ...
                if (Context.class.isAssignableFrom(targetClass)) {
                    // 如果当前名称是目录（Context 实现类：NamingContext）的话，递归查找
